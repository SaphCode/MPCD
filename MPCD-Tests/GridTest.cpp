...
TEST(Grid, Constants) {
	...
	std::string num("av" + std::to_string(average_particles_per_cell));
	std::string csv(".csv");
...
TEST(Grid, AverageNumberPerCell) {
	...
	for (int i = 0; i < MPCD::Constants::number; i++) {
		...
		frequencies[std::make_pair(cell_index[0], cell_index[1])] += 1;
	...
	std::string num("_av" + std::to_string(average_particles_per_cell));
	std::string csv(".csv");
	...
	for (auto const& [key, value] : frequencies) {
		outFile << key.first << "," << key.second << "," << value << "\n";
...
TEST(Grid, MeanVelocity_OneParticlePerCell) {
	...
	const int rows = MPCD::Constants::Pipe::height / MPCD::Constants::Grid::cell_dim;
	const int cols = MPCD::Constants::Pipe::width / MPCD::Constants::Grid::cell_dim;
	...
	* This block should place 1 particle in each cell. The mean of every cell should then be the velocity of the particle itself.
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			Vector2i index(i, j);
			totalCellVelocities.insert(std::make_pair(std::make_pair(index[0], index[1]), zero));
		}
	}
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			...
			totalCellVelocities[std::make_pair(i, j)] += vel;
			cellParticles[std::make_pair(i, j)] += 1;
		}
	}
...
TEST(Grid, MeanVelocity_AllParticlesInOneCell) {
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < cols; j++) {
			Vector2i index(i, j);
			totalCellVelocities.insert(std::make_pair(std::make_pair(i, j), zero));
		}
	}
	...
	for (int i = 0; i < num; i++) {
		Particle p(pos, vel);
		particles.push_back(p);
		Vector2i index = p.getCellIndex();
		totalCellVelocities[std::make_pair(index[0], index[1])] += vel;
		cellParticles[std::make_pair(index[0], index[1])] += 1;
	}
